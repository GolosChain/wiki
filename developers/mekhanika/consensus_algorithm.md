...
_Экономическая заинтересованность в оптимизации необходимых компьютерных алгоритмов Модель распределения, которая привлекает технически грамотных пользователей Создание надежной сети с малой временной задержкой_
...
Пусть HASH = надёжная хэш-функция \(SHA256 или лучше\) Пусть H = заголовок блока ID Пусть Н2 = HASH \(H + NONCE\) Пусть PRI = приватный ключ производителя Пусть PUB = публичный ключ производителя Пусть S = SIGN \(PRI, HASH \(H\)\) Пусть K = RECOVER\_PUBLIC\_KEY \(H2, S\) Пусть POW = HASH \(K\)
