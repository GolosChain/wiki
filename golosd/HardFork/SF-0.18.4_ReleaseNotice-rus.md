...
В версии SF-0.18.4 введена новая виртуальная (не заданная явным образом) операция, генерируемая на каждом из блоков. Операция оповещает пользователя о вознаграждении в виде GESTS подписчика блока — продюсера блока, которым может являться одно из следующих лиц:  
...
`ignore_messages_from_unknown_contact` — "false" для прекращения получения сообщений от неизвестного контакта. 
...
> Доработка выполнена по просьбе делегатов.  
